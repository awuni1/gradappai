...
//         if (urlError === 'invalid_request' && errorCode === 'bad_oauth_state') {
//           userFriendlyMessage =
//             "OAuth session expired or invalid. This may happen if you're redirecting to the wrong domain. Please try signing in again.";
//         } else if (urlError === 'access_denied') {
//           userFriendlyMessage =
//             'You declined to authorize the application. Please try again if you want to sign in.';
//         } else if (urlError === 'invalid_request') {
//           userFriendlyMessage =
//             'Invalid OAuth request. This may be due to a configuration mismatch. Please try again.';
//         } else if (errorDescription) {
//           userFriendlyMessage = decodeURIComponent(errorDescription.replace(/\+/g, ' '));
//         }
...
//       // 2) Fallback: legacy hash-based flows
//       if (!processed) {
//         if (hashParams.get('access_token') || hashParams.get('refresh_token')) {
//           try {
//             window.history.replaceState({}, document.title, window.location.pathname);
...
//       try {
//         const redirectUrl = await withTimeout(authService.getPostAuthRedirect(user.id), 5000, 'getPostAuthRedirect');
//         navigate(redirectUrl);
//       } catch (error) {
//         console.warn('Redirect decision timed out or failed, defaulting to dashboard:', error);
//         navigate('/dashboard');
...
//               <Alert className="mt-4 border-green-200 bg-green-50">
//                 <CheckCircle className="h-4 w-4" />
//                 <AlertDescription className="text-sm">Taking you to your dashboard...</AlertDescription>
//               </Alert>
//             )}
//           </div>
//         </div>
...
const AuthCallback: React.FC = () => {
  ...
  const handleAuthCallback = async () => {
    try {
      ...
      // 1️⃣ Handle OAuth errors in URL
      if (urlError) {
        console.error('OAuth URL Error:', { urlError, errorDescription });
        setStatus('error');

        const userFriendlyMessage = decodeURIComponent(
      ...
      // 3️⃣ If no session, exchange code for session
      if (!session) {
        const result = await (supabase.auth as any).exchangeCodeForSession(window.location.href);
        if (result.error) throw result.error;

        // Fetch session again after successful exchange
      ...
      // 4️⃣ Ensure user profile exists
      try {
        const { profile } = await authService.getUserProfile(user.id);
        if (!profile) {
          const profileData = {
      ...
      // 5️⃣ Redirect to post-auth page or dashboard
      try {
