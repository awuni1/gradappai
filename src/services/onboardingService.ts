...
export const onboardingService = {
  /**
   * Save progress locally (fast + non-blocking).
   */
  async saveProgress(data: OnboardingData) {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
    } catch (err) {
      console.warn('Failed to save progress locally:', err);
    }
  },

  /**
   * Load progress locally (instant).
   * Fallback: try CV analysis in background.
   */
  async loadProgress(): Promise<OnboardingData | null> {
    try {
    ...
    // 🚀 Defer CV analysis (don’t block UI)
    setTimeout(async () => {
      try {
        const { data: { user } } = await supabase.auth.getUser();
        if (!user) return;

        const cvProfile = await cvAnalysisService.getEnhancedUserProfile(user.id);
        if (cvProfile) {
          const cvData: Partial<OnboardingData> = {
            firstName: cvProfile.firstName || '',
            lastName: cvProfile.lastName || '',
            currentDegree: cvProfile.degree || '',
            currentInstitution: cvProfile.institution || '',
            gpa: cvProfile.gpa?.toString() || '',
            targetField: cvProfile.field_of_study || '',
            targetDegreeLevel: cvProfile.target_degree_level || ''
          };
          localStorage.setItem(STORAGE_KEY, JSON.stringify(cvData));
        }
      } catch (err) {
        console.warn('Background CV load failed:', err);
      }
    }, 0);
  ...
  /**
   * Mark onboarding complete quickly, then sync heavy stuff in background.
   */
  async completeOnboarding(data: OnboardingData): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          if (data.researchInterests?.length) {
            // You must resolve research_interest_id for each interest before upserting
            for (const interest of data.researchInterests) {
              // Find or create the research interest in research_interests table
              let { data: researchInterest, error: selectError } = await supabase
                .from('research_interests')
                .select('id')
                .eq('name', interest)
                .maybeSingle();
              ...
              if (!researchInterest) {
                const { data: newInterest, error: createError } = await supabase
                  ...
                  .select('id')
                  .single();
                researchInterest = newInterest;
              }

              if (researchInterest?.id) {
                await supabase.from('user_research_interests').upsert([{
                  user_id: user.id,
                  research_interest_id: researchInterest.id,
                  updated_at: new Date().toISOString()
                }],  { onConflict: 'user_id,research_interest_id' });
              }
            }
          }
        } catch (err) {
          console.warn('Background onboarding sync failed:', err);
        }
      }, 0);
      ...
      return { success: false, error: 'Unexpected error during onboarding' };
    }
  },

  /**
   * Quick check if onboarding completed (uses localStorage first).
   */
  async hasCompletedOnboarding(): Promise<boolean> {
    try {
      const localFlag = localStorage.getItem(ONBOARDING_COMPLETE_KEY);
